...
// Registro de usuario utilizando async/await
async function registerUser(user) {
    try {
        const validatedUser = await validateForm(user);
        const storedUser = await storeInDatabase(validatedUser);
        const message = await sendWelcomeEmail(storedUser);
        console.log(message);
    } catch (err) {
